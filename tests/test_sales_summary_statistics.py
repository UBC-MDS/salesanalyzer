...
def sample_data():
    return pd.DataFrame({
        'Quantity': [10, 5, 5, 20, 5],
        'UnitPrice': [100, 200, 150, 100, 100],
...
def test_sales_summary_statistics_normal_data(sample_data):
    ...
    expected = pd.DataFrame({
        ...
        'unique_customers': [3],
        'average_order_value': [1050.0],
        'top_selling_product_quantity': ['Product A, Product C'],  # Tied top-selling products
        'top_selling_product_revenue': ['Product A'],  # Only Product A has the highest revenue
        'average_revenue_per_customer': [1750.0]
